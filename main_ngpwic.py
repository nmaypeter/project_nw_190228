from SeedSelection_NGPW import *
            ...
            for prod_setting in prod_setting_seq:
                for prod_setting2 in prod_setting2_seq:
                    ...
                    for bud in range(begin_budget, total_budget + 1):
                        ...
                        ssngpw_main = SeedSelectionNGPW(graph_dict, seed_cost_dict, product_list, bud, monte_carlo)
                        ...
                        for sample_count in range(sample_number):
                            ...
                            prod_wei_list = ssngpw_main.getProductWeight()
                            celf_sequence = ssngpw_main.generateCelfSequence(prod_wei_list)
                            mep_celf = [-1, 0.0]
                            ...
                            while now_budget <= bud and mep_i_node != '-1':
                                ...
                                if mep_flag == seed_set_length:
                                    ...
                                    mep_mg_score = round(mep_mg * prod_wei_list[mep_k_prod], 4)
                        ...
                        for pps in pps_seq:
                            ...
                            for sample_count, sample_seed_set in enumerate(seed_set_sequence):
                                print("@ mngpwic evaluation @ data_set_name = " + data_set_name + ", wpiwp = " + str(wpiwp) +
                            ...
                            path1 = "result/mngpwic_pps" + str(pps) + "_wpiwp" * wpiwp
                            if not os.path.isdir(path1):
                                os.mkdir(path1)
                            path = "result/mngpwic_pps" + str(pps) + "_wpiwp" * wpiwp + "/" + data_set_name + "_" + product_name
                            if not os.path.isdir(path):
                                os.mkdir(path)
                            fw = open(path + "/" + "b" + str(bud) + "_i" + str(sample_number) + ".txt", 'w')
                            fw.write("mngpwic, pp_strategy = " + str(pps) + ", total_budget = " + str(bud) + ", wpiwp = " + str(wpiwp) + "\n" +
